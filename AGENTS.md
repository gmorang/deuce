...
- **Rankings are private / invite-only.** Read of a ranking and all its subcollections = member or admin. The home lists only your memberships (`useMyRankings` → collection-group query on `members` where `uid == me`; admins see all). You join with a code: `rankings/{id}.inviteCode` + a public `inviteCodes/{code}` → `{ rankingId, startRating }` lookup doc (`useJoinByCode`). Member docs carry a `uid` field (needs the collection-group index in `firestore.indexes.json`).
- `rankings/{id}`: `name, description?, icon, color, ownerId, archived, inviteCode, createdAt, settings {startRating, kFactor, provisionalMatches}`. Only admins create/update/delete. Admin-only Settings screen at `/r/:id/config` edits identity + Elo settings + archive/delete. Elo settings thread into join (startRating), record-match (kFactor, read in the transaction), and leaderboard (provisional badge). Rules validate a member's starting rating against the ranking's `startRating`.
